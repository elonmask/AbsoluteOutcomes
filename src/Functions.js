    ...
    statistics.events.forEach((event, idx) => {
      if (event.text.includes(setNumber) && event.text.includes(" set")) {
        for (let i = idx; i < statistics.events.length - idx; i++) {
          ...
          if (statistics.events[i].text.includes(setNumber + 1) && event.text.includes(" set")) {
            break;
          }
          if (statistics.events[i].text.split(" ")[statistics.events[i].text.split(" ").length - 1] === "Game") {
            gamesCounter++;
          }
          if (gamesCounter === gameNumber) {
           if (player === "home") {
             try {
               if (total > parseInt(statistics.events[i]?.extra?.game_points.home)) {
                 market.outcomes[0].status = 2;
                 market.outcomes[1].status = 3;
               }
               if (total < parseInt(statistics.events[i]?.extra?.game_points.home)) {
             ...
             try {
               if (total > parseInt(statistics.events[i]?.extra?.game_points.away)) {
                 market.outcomes[0].status = 2;
                 market.outcomes[1].status = 3;
               }
               if (total < parseInt(statistics.events[i]?.extra?.game_points.away)) {
        ...
        if (gamesCounter < gameNumber && Object.keys(statistics.periods).length + 1 === setNumber) {
    ...
    if (Object.keys(statistics.periods).length >= set) {
      ...
      statistics.events.forEach((event, idx) => {
        if (event.text.includes(set) && event.text.includes(" set")) {
          for (let i = idx; i < statistics.events.length - idx; i++) {
            console.log("test")
            if (statistics.events[i].text.includes(set + 1) && event.text.includes(" set")) {
              break;
            }
            if (statistics.events[i].text.split(" ")[statistics.events[i].text.split(" ").length - 1] === "Game") {
              gamesCounter++;
...
const MatchBothPlayersWinSet = (statistics, market) => {
  if (statistics.time_status === "3") {
    if (statistics.result.home > statistics.result.away && statistics.result.home > 1 && statistics.result.away > 1) {
      market.outcomes[0].status = 2;
      market.outcomes[1].status = 3;
    }
    if (statistics.result.away > statistics.result.home && statistics.result.home > 1 && statistics.result.away > 1) {
      market.outcomes[0].status = 3;
      market.outcomes[1].status = 2;
    }
...
const PlayerTotalGames = (statistics, market) => {
  const player = market.name.includes("competitor1") ? statistics.home.name : statistics.away.name;
  const total = parseFloat(market.specifiers.total);

  if (statistics.time_status === "3") {
    let counter = 0;
    statistics.events.forEach(event => {
      if (event.text.split(" ")[event.text.split(" ").length - 1] === "Game" && event.text.includes(player)) {
        counter++;
      }
    })
...
const AnySetWillEndSix = (statistics, market) => {
  ...
  if (statistics.time_status === "3") {
    Object.keys(statistics.periods).forEach(period => {
      if (period.home === 6 && period.away === 0 || period.home === 0 && period.away === 6) {
        isTrue = true;
      }
    })
...
const SetWinner = (statistics, market) => {
  ...
  if (statistics.periods[`p${setNumber}`]) {
    const winner = statistics.periods[`p${setNumber}`].home > statistics.periods[`p${setNumber}`].away ? "home" : "away";
...
const SetCorrectScore = (statistics, market) => {
  ...
  if (statistics.periods[`p${setNumber}`]) {
    const score = statistics.periods[`p${setNumber}`].home + ":" + statistics.periods[`p${setNumber}`].away;
    market.outcomes.forEach((outcome) => {
      if (outcome.outcome === score) {
        outcome.status = 2;
      } else {
